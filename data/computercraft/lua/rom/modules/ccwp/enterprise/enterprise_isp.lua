...
function enterprise_isp.StoreItemsFrom_ASrv(...)
    -- get & check input from description
    local checkSuccess, itemsLocator, itemDepotLocator, assignmentsPriorityKey, callback = InputChecker.Check([[
        ...
        Parameters:
            ...
            callback                    + (Callback) to call once service is ready
    --]], table.unpack(arg))
    if not checkSuccess then corelog.Error("enterprise_isp.StoreItemsFrom_ASrv: Invalid input") return Callback.ErrorCall(callback) end

    -- get obj
    local obj = Host.GetObject(itemDepotLocator)
    if type(obj) ~= "table" then corelog.Error("enterprise_isp.StoreItemsFrom_ASrv: obj "..itemDepotLocator:getURI().." not found.") return Callback.ErrorCall(callback) end
...
function enterprise_isp.Can_StoreItems_QSrv(...)
    -- get & check input from description
    local checkSuccess, itemsLocator = InputChecker.Check([[
        ...
        Parameters:
            serviceData             - (table) data to the query
                itemsLocator        + (URL) locating the items that need to be stored
                                        ...
                                        (the "query" component of the URL specifies the items to query for)
    --]], table.unpack(arg))
    if not checkSuccess then corelog.Error("enterprise_isp.Can_StoreItems_QSrv: Invalid input") return {success = false} end

    -- get obj
    local itemDepotLocator = itemsLocator:baseCopy()
    local obj = Host.GetObject(itemDepotLocator)
    if type(obj) ~= "table" then corelog.Error("enterprise_isp.Can_StoreItems_QSrv: obj "..itemDepotLocator:getURI().." not found.") return {success = false} end
...
function enterprise_isp.NeedsTo_StoreItemsFrom_SSrv(...)
    -- get & check input from description
    local checkSuccess, itemsLocator, itemDepotLocator = InputChecker.Check([[
        ...
        Parameters:
            serviceData                         + (table) data to the query
                ...
                itemDepotLocator                + (URL) locating the ItemDepot where the items need to be stored to
    --]], table.unpack(arg))
    if not checkSuccess then corelog.Error("enterprise_isp.NeedsTo_StoreItemsFrom_SSrv: Invalid input") return {success = false} end

    -- get obj
    local obj = Host.GetObject(itemDepotLocator)
    if type(obj) ~= "table" then corelog.Error("enterprise_isp.NeedsTo_StoreItemsFrom_SSrv: obj "..itemDepotLocator:getURI().." not found.") return {success = false} end
